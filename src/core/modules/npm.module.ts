...
export default function (bot: Telegraf) {
  bot.command("/npm", async (ctx) => {
    let query: string = ctx.message.text.replace("/npm ", "");
    if (query.includes("?") == false) {
      try {
        ...
        if (html.status == 200) {
          ...
          let btns = await urlButtons(buttons, 1);
          ctx.replyWithHTML(message, btns);
        }
      } catch (error) {
        ctx.replyWithMarkdownV2(`*Error, modulo no encontrado*`);
      }
    } else {
      let query = ctx.message.text.replace("/npm ? ", "");
      try {
        let header = {
          headers: {
            "User-Agent":
