import {
  ...
  delete_bio,
  get_bio,
} from "../controllers/bios.controller";
  ...
  bot.command("/bio", async (ctx) => {
    ...
    try {
      ...
      if (
        ...
        // @supergrupo y mensage de respuesta
    ...
    try {
      ...
      if (
        ...
        if (emisor.id == receptor.id) {
          // Evita la autobografia
          ctx.reply("No puedes crear tu propia biografia", {
            reply_to_message_id: message_id,
          });
          return;
        }
        if (emisor.id == owner.id && receptor.id == _bot.id) {
          // Da acceso al dueño
          bio.text = text;
        ...
        if (receptor.id == _bot.id) {
          // Protege al bot
          ctx.reply("No cuentas con los permisos necesarios", {
            reply_to_message_id: message_id,
          });
          return;
        }
        if (receptor.id == owner.id) {
          // Proteje al dueño
